...
In questo lab verranno implementate le funzionalità di Rete per la distribuzione di contenuti di Azure per fornire una soluzione di memorizzazione nella cache in base alle posizioni dei clienti. Il lab configura un account di archiviazione per i file di immagine e video, che hanno effetto sulla maggior parte dei problemi di latenza. Si userà la rete per la distribuzione di contenuti di Azure per implementare la soluzione di memorizzazione nella cache per facilitare la latenza per queste immagini e file video.
...
1. Nella finestra apri browser passare alla portale di Azure in `https://portal.azure.com`e quindi accedere con l'account che si userà per questo lab.
...
1. Nel pannello **Panoramica servizio app**, in **Essentials**, registrare il valore del collegamento **URL**. Questo valore verrà utilizzato più avanti nel lab.
...
1. Nella scheda **Profilo CDN** del pannello **Informazioni principali** eseguire le azioni seguenti e selezionare **Rivedi e crea**:
   ...
   | **Creare una nuova casella di controllo endpoint della rete CDN** | Non selezionato |
...
1. Nella finestra popup **Carica BLOB** eseguire le azioni seguenti:
    ...
    a.  Nella sezione **File** selezionare **Cerca file** o usare la funzionalità di trascinamento della selezione.

...
In questo lab si inizierà con un'app Web di prova, ospitata in un contenitore, che verrà usata per sottoscrivere griglia di eventi. Questa app ti consentirà di inviare eventi e ricevere messaggi di conferma che gli eventi hanno avuto esito positivo.
...
1. Nella finestra del browser aperto passare al portale di Azure all'indirizzo `https://portal.azure.com`e quindi accedere con l'account che verrà usato per questo lab.
...
1. Nel pannello **PubSubEvents** selezionare l'app Web **eventviewer**_[yourname]_ , verrà visualizzata la sezione **Panoramica** .

1. Nella sezione **Panoramica** registrare il valore del **dominio predefinito** nel riquadro essentials. Questo valore verrà utilizzato più avanti nel lab.
...
1. Aggiungere quanto segue di codice:
...
1. Aggiungere il codice seguente nel metodo **Main** del file **Program.cs** :
    ...
    > **Nota**: per altre informazioni su Griglia di eventi, fare clic sui collegamenti sconfisse: 

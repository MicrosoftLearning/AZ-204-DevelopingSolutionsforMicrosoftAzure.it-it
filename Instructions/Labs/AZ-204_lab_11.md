...
1. Eseguire il comando seguente per importare la versione 2.21.0 di **Microsoft.ApplicationInsights** da NuGet al progetto corrente:
...
1. Al prompt del terminale eseguire il comando seguente per importare la versione 2.21.0 di **Microsoft.ApplicationInsights.PerfCounterCollector** da NuGet al progetto corrente:
...
1. Nel riquadro dell'editor **** individuare ed eliminare il codice seguente nella riga 17:
...
1. **Nel file Program.cs** aggiungere le due righe seguenti a partire dalla riga 7 per abilitare Profiler registrandolo insieme ad Application Insights nella raccolta di servizi:
...
1. Nel riquadro Esplora risorse della **finestra **di Visual Studio Code** passare alla **directory bin\Debug\net8.0**.**

    > **Nota**: per verificare di essere nella **directory bin\Debug\net8.0**, in Visual Studio Code scegliere Apri cartella dal menu File e passare a **Tuttifile (F):\\Allfiles\\Labs\\11\\Starter\\API\\\\Release\\net8.0\win-x86\publish** e quindi selezionare **Seleziona cartella**.

1. Nella finestra di **Visual Studio Code**, nella barra dei menu, selezionare **Terminale** e quindi **Nuovo terminale**.

1. Al prompt del terminale eseguire il comando seguente per assicurarsi che la directory corrente sia impostata su **Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\Bin\\Release\\net8.0\\net8.0\\win-x86\\**, dove risiedono i file di distribuzione:

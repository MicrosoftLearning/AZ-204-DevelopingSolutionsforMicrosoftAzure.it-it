...
Inoltre, poiché Contoso potrebbe voler accedere ai dati in Cosmos DB tramite un'interfaccia intuitiva, si implementerà una soluzione .NET che accede e visualizza i dati di Cosmos DB in un Web browser. 

Infine, si imposterà il livello di coerenza per l'istanza di Cosmos DB e si implementerà una funzione di Azure per le notifiche del feed di modifiche.
...
1. Nella finestra del browser aperto passare alla portale di Azure all'indirizzo `https://portal.azure.com`e quindi accedere con l'account che verrà usato per questo lab.
...
1. Dalla scheda dell'editor di codice per il file **AdventureWorksCosmosContext.cs** aggiungere il codice seguente:

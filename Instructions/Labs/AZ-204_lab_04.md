...
1. Nella **scheda Informazioni di base** della **pagina Crea account Azure Cosmos DB - Azure Cosmos DB per NoSQL** eseguire le azioni seguenti e quindi selezionare **Rivedi e crea**:
...
1. Nella **scheda Rivedi e crea** della **pagina Crea account Azure Cosmos DB - Azure Cosmos DB per NoSQL** esaminare le opzioni selezionate durante i passaggi precedenti.
...
1. **Nel riquadro Chiavi**, nella **scheda Chiavi** di lettura/scrittura, registrare i valori delle caselle di **testo URI**, **PRIMARY KEY** e **PRIMARY CONNECTION STRING** in Blocco note. Questi valori verranno utilizzati più avanti in questo lab.
...
1. **Nella sezione Contenitori** passare al contenitore di immagini** appena creato**.

1. Trovare la sezione **Impostazioni** nel pannello **Contenitore** e quindi selezionare il collegamento **Proprietà**.

1. **Nel riquadro Proprietà** prendere nota e registrare il valore nella casella di testo URL** in **Blocco note. Questo valore verrà usato più avanti nel lab.

1. Fare clic su **Token di accesso condiviso e generare un token di accesso con le impostazioni predefinite facendo clic su **Genera token di firma di accesso** condiviso e URL**. Registrare il valore del **token** di firma di accesso condiviso BLOB in Blocco note, sarà necessario in un secondo momento.

   > **Nota**: è ora necessario avere 5 valori in Blocco note. **URI**, **chiave** pimary e **stringa** di Connessione primaria per l'account Cosmos DB. Quindi l'URL** e **il **token** di firma di accesso condiviso per il contenitore BLOB.
...
1. **Nel file Program.cs**, alla riga 14, impostare il valore di **EndpointUrl** sostituendo la stringa vuota con la **proprietà URI** dell'account **** Cosmos DB registrato in precedenza in questo lab. Assicurarsi che il valore sia racchiuso tra virgolette doppie. Questo è il **primo** valore registrato in Blocco note.

1. Nella riga 15 impostare il valore di **AuthorizationKey** sostituendo la stringa vuota con la **proprietà PRIMARY KEY** dell'account **** Cosmos DB registrato in precedenza in questo lab. Assicurarsi che il valore sia racchiuso tra virgolette doppie. Questo è il **secondo** valore registrato in Blocco note.
...
1. Aggiornare il valore della proprietà **AdventureWorksCosmosContext** impostandone il valore sulla **STRINGA DI CONNESSIONE PRIMARIA** dell'account Azure Cosmos DB registrato in precedenza in questo lab. Questo è il **terzo** valore registrato in Blocco note.

1. Nell'oggetto JSON alla riga 6 trovare il **Impostazioni. Proprietà BlobContainerUrl**. Si noti che il valore corrente è vuoto:
...
1. Aggiornare le **proprietà BlobContainerUrl** e **BlobSASToken** impostandone il valore sulla **proprietà URL** del contenitore BLOB Archiviazione di Azure denominata **images** e il valore del token** di firma di accesso condiviso del BLOB **registrato in precedenza in questo lab di conseguenza. Questi erano i quattro** e **i **quinto** valori registrati in Blocco note.

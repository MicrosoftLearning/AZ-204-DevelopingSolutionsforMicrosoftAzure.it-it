...
1.  Nel pannello **Seleziona l'opzione API** selezionare **Core (SQL) - Consigliato**.
...
1.  Nel portale di Azure tornare al pannello **Azure Cosmos DB** e nell'elenco di account Cosmos DB selezionare l'account **polycosmos** _[nomeutente]_ appena creato.
...
1.  Nella scheda **Rivedi e crea** del pannello **Crea un account di archiviazione** esaminare le opzioni selezionate durante i passaggi precedenti.
...
1.  Nella finestra di **Visual Studio Code** selezionare un'area vuota del riquadro **Esplora risorse**, attivare il menu di scelta rapida e quindi selezionare **Apri nel terminale integrato**.

1.  Dal prompt del terminale verificare che la directory corrente sia impostata su **AdventureWorks** (o modificarla in caso contrario), quindi eseguire il comando seguente per commutare il contesto del terminale sulla cartella **AdventureWorks.Upload**:
...
1.  Dal prompt del terminale eseguire il comando seguente per aggiungere la libreria client .NET di Azure Cosmos DB al progetto attualmente aperto:
    ...
    > **Nota**: il comando **dotnet add package** aggiungerà il pacchetto **Microsoft.Azure.Cosmos** da **NuGet**. Per altre informazioni, vedere [Microsoft.Azure.Cosmos](https://www.nuget.org/packages/Microsoft.Azure.Cosmos/3.20.1).
...
1.  Nella riga 18 impostare il valore di **PrimaryKey** sostituendo la stringa vuota con **"/Category"** .
...
1.  Dal prompt del terminale eseguire il comando seguente per ripristinare eventuali pacchetti NuGet mancanti e compilare il progetto nella cartella:
...
1.  Nel portale di Azure tornare al pannello **Azure Cosmos DB** e nell'elenco di account Cosmos DB selezionare l'account **polycosmos** _[nomeutente]_ creato in precedenza in questo lab.

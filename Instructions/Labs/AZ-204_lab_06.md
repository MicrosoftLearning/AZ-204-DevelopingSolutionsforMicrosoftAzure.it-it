...
## Assicurarsi che l'orario dell'ambiente lab locale sia impostato correttamente (nessun ritardo in base al fuso orario)
...
In questo lab si registrerà un'applicazione in Azure AD, si aggiungerà un utente e quindi si testerà l'accesso dell'utente all'applicazione per verificare che Azure AD possa proteggerlo. Si userà anche lo strumento Graph Explorer per compilare e testare le richieste in base al API Graph per un account utente di Azure AD.
...
### Esercizio 1: Configurare un ambiente di Azure AD a singolo tenant
...
#### Attività 2: Registrare un'applicazione in Azure AD

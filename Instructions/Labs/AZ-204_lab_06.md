...
1. Nella portale di Azure selezionare l'icona **di Cloud Shell icona ![](./media/az204_lab_CloudShell.png) di Cloud Shell** per aprire cloud shell. Se Per impostazione predefinita Cloud Shell è una sessione Bash, selezionare **Passa a PowerShell** nel **menu di Cloud Shell** e quindi selezionare **Conferma**.
    ...
    | Richiesta | Azione |
    |--|--|
    | Benvenuto in Azure Cloud Shell | Selezionare **PowerShell**. |
...
1. Eseguire il comando seguente per recuperare e visualizzare il nome di dominio DNS (Domain Name System) primario del tenant di Microsoft Entra:
...
1. Eseguire il comando seguente per identificare il nome dell'entità utente (UPN) dell'utente microsoft Entra ID appena creato:
...
1. Passare alla cartella **Proprietà**, aprire il file **launchSettings.json** e quindi applicare le modifiche seguenti:
    ...
    > **Nota**: i numeri di porta devono corrispondere al valore specificato durante la creazione della registrazione dell'app Microsoft Entra ID.
...
1. Verificare che la `Microsoft.AspNetCore.Authentication.JwtBearer` versione dei pacchetti NuGet e `Microsoft.AspNetCore.Authentication.OpenIdConnect` sia impostata su **8.0.0**.
...
1. Nella finestra del browser aperta, quando richiesto, eseguire l'autenticazione usando l'UPN **del aad_lab_user1** account ID Microsoft Entra creato in precedenza in questo lab con **Pa55w.rd1234** come password.

    > **Nota**: se viene visualizzata la finestra **Aiutaci a proteggere il tuo account**, selezionare **Ignora per adesso**.

    > **Nota**: se viene visualizzato il messaggio **Richiedere l'approvazione** dell'amministratore, sarà necessario usare un ID Microsoft Entra con privilegi elevati per completare la parte restante di questa attività. A tale scopo, selezionare il collegamento **Avere un account amministratore? Accedere con tale account**, accedere con un account amministratore e procedere con il passaggio successivo.
...
1. Esaminare la **** home page iniziale del sito di destinazione visualizzato dal browser e verificare che l'UPN **del aad_user1** account ID Microsoft Entra venga visualizzato nella finestra del browser.

1. Nella pagina **Introduzione** selezionare **Disconnetti**.

1. Quando viene richiesto di selezionare l'account da disconnettere, selezionare il **aad_lab_user1** account ID Microsoft Entra. Si verrà reindirizzati automaticamente alla pagina **Disconnesso**.

1. Chiudere il browser Microsoft Edge.

#### Revisione 

In questo esercizio è stata implementata un'app Web a tenant singolo e testata in un ambiente Microsoft Entra a tenant singolo.

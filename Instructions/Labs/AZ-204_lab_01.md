...
1. Se nella sezione Configurazione è presente la scheda **Applicazione Impostazioni****, eseguire le azioni seguenti, selezionare **Salva** e quindi selezionare **Continua**.**

   | Impostazione                                        | Azione                                                                                                                  |
   | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
   | Scheda **Impostazioni applicazione**                   | Selezionare **+ Nuova impostazione dell'applicazione**                                                                                    |
   ...
   | Casella di controllo **Impostazione slot di distribuzione**          | Mantenere il valore predefinito e quindi selezionare **OK** per chiudere la finestra di dialogo popup e tornare alla sezione **Configurazione** |
...
1. Se nella sezione Configurazione** non **è presente la **scheda Impostazioni applicazione**, controllare se nella sezione Impostazioni** sono presenti i collegamenti **Variabili** **di ambiente, selezionarlo ed eseguire le azioni seguenti:

   | Impostazione                                        | Azione                                                                                                                  |
   | ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
   | **Scheda Impostazioni** app                           |  Seleziona                                                                                                                 |
   | Prima casella di testo disponibile nella **colonna Nome** |  **Archiviazione Connessione ionString**                                                                                            |   
   | Casella di testo nella **colonna Valore**                | Incollare la stringa di connessione dell'archiviazione copiata in precedenza in Blocco note                                               |
   | Fare clic su **Applica** nella parte inferiore                  | Verrà aperto il popup di conferma                                                                                  | 
   | Fare clic su **Conferma** in confirmatioj Po-up | In questo modo verrà salvato il valore di configurazione appena immesso |

   Prima di procedere con il lab, attendere il completamento del salvataggio delle impostazioni dell'applicazione.

1. **Nel pannello servizio app** della sezione Impostazioni selezionare il **collegamento Proprietà** per visualizzare altre informazioni sulle servizio app.

1. Per ottenere l'URL del servizio app, passare al **collegamento Panoramica**, copiare il valore dalla **sezione Dominio predefinito** e incollarlo in Blocco note. Anteporre `https://` al nome di dominio in Blocco note. Questo valore verrà usato più avanti nel lab.
...
1. Nella classe ImagesController** alla **riga 68 osservare il **metodo Post** e il codice usato per rendere persistente un'immagine caricata in Archiviazione.

1. Sulla barra delle applicazioni selezionare l'icona **Terminale** .

1. Nel terminale aperto immettere il comando seguente e quindi premere INVIO per accedere al interfaccia della riga di comando di Azure (interfaccia della riga di comando):
...
1. Nel terminale immettere il comando seguente e quindi selezionare Invio per elencare tutte le app nel **gruppo di risorse ManagedPlatform** :
...
1. Chiudere le applicazioni di Visual Studio Code e Terminal attualmente in esecuzione.
...
1. Nella pagina Web **Contoso Photo Gallery**, nella sezione **Upload a new image** (Carica una nuova immagine) eseguire le azioni seguenti:
   ...
   b. **Nella finestra Esplora file** passare a **Allfiles (F):\\Allfiles Labs 01\\Starter\\Images** (Immagini iniziali di Allfiles\\Labs\\), selezionare il **file banhmi.jpg** e quindi selezionare **Apri**.

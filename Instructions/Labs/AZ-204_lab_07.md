...
1. Nella **scheda Impostazioni** app selezionare **+ Aggiungi**. Immettere le informazioni seguenti nella **finestra di dialogo popup Aggiungi/Modifica impostazione** applicazione:
    ...
    | Casella di testo **Valore** | Costruire un valore usando la sintassi seguente: `@Microsoft.KeyVault(SecretUri=<Secret Identifier>)`, dove il `<Secret Identifier>` segnaposto rappresenta l'identificatore del segreto registrato in precedenza in questo esercizio. |

...
1. Nel pannello **Crea app** per le funzioni **, nella **scheda Informazioni di base, eseguire le azioni seguenti e quindi selezionare **Avanti: Archiviazione**:
    ...
    | Elenco a discesa **Versione** | Selezionare **8 (LTS) Isolato (anteprima)** |
...
1. Nella riga 17 sostituire `"0 */5 * * * *"` con '"0 */2 * * * *"'' per impostare l'intervallo di frequenza ricorrente su 2 minuti anziché 5 minuti e salvare la modifica.
...
1. Osservare l'esecuzione della funzione che si verifica circa ogni due minuti. Ogni esecuzione della funzione deve eseguire il rendering di un messaggio semplice nel log.
1. Chiudere l'applicazione **Terminale Windows** attualmente in esecuzione.
...
#### Attività 3: Scrivere codice della funzione attivata da HTTP con input BLOB
...
1. Dal prompt dei comandi eseguire il comando seguente per registrare l'estensione [Microsoft.Azure.Functions.Worker.Extensions.Archiviazione](https://www.nuget.org/packages/Microsoft.Azure.Functions.Worker.Extensions.Storage/6.2.0):
...
1. Sulla barra delle applicazioni selezionare di nuovo l'icona di **Terminale Windows** per aprire una nuova istanza dell'applicazione **Terminale Windows**.
1. Dal prompt dei comandi eseguire il comando seguente per testare la chiamata ALL'API **REST POST** con il corpo della richiesta HTTP impostato su un valore numerico pari **a 3**:`http://localhost:7071/api/echo`

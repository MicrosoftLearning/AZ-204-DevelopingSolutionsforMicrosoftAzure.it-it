...
Individuare la barra delle applicazioni nel desktop di Windows 11. La barra delle applicazioni include le icone per le applicazioni che verranno usate nel lab, tra cui:
...
In questo lab si esplorerà come creare e distribuire contenitori nel Registro Azure Container, usando un'applicazione .NET e i file Docker. Si distribuirà anche una soluzione in contenitori in App contenitore di Azure.
...
1. Eseguire il comando seguente per creare nella directory corrente un nuovo file denominato **Dockerfile**:
...
1. Eliminare il file **Starter.sln**.
    ...
    > **Nota**: In questo modo si elimina la necessità di fare esplicitamente riferimento al file .csproj quando si usano i comandi dotnet.

1. Passare all'applicazione **Terminale**.

1. Usare il comando seguente per eseguire l'applicazione:

    ```powershell
    dotnet run
    ```

1. Esamina i risultati. Deve essere presente almeno un indirizzo IP, che rappresenta l'indirizzo IP assegnato al computer in uso.
...
1. Salvare il file **Dockerfile**.
    ...
    > **Nota**: si creerà quindi un archivio contenente tutti i file di progetto. In questo modo sarà più semplice caricare i file di progetto in Cloud Shell.
...
1. Nel portale di Azure selezionare l'icona di **Cloud Shell** ![Icona di Cloud Shell](./media/az204_lab_CloudShell.png) per aprire Cloud Shell. Se per impostazione predefinita Cloud Shell è una sessione di PowerShell, selezionare **Passa a Bash** nel menu di **Cloud Shell** e quindi selezionare **Conferma**.

    > **Nota**: l'icona **Cloud Shell** è rappresentata da un segno di maggiore (\>) e un carattere di sottolineatura (\_).

    Se è la prima volta che si avvia **Cloud Shell**, verrà visualizzata una serie di richieste per creare la risorsa. Eseguire le azioni indicate nella tabella seguente:

    | Richiesta | Azione |
    |--|--|
    | Benvenuto in Azure Cloud Shell | Selezionare **Bash**. |
    | Introduzione | Selezionare **Montare l'account di archiviazione**, quindi selezionare la **Sottoscrizione dell'account di archiviazione ** nell'elenco a discesa e infine selezionare **Applica**. |
...
1. Nella finestra di dialogo **Apri** passare alla directory vuota **Allfiles (F):\\Allfiles\\Labs\\05\\Starter**, selezionare **lab05.zip** e quindi **Apri**.
...
1. Eseguire il comando seguente per passare dalla directory corrente alla directory **\~/ipcheck**:
...
### Esercizio 2: Distribuire un'istanza di Azure Container

#### Attività 1: Abilitare l'utente amministratore in Registro Container

1. Nel gruppo di risorse **ContainerCompute** selezionare il Registro Container creato in precedenza in questo lab e quindi selezionare il pannello **Chiavi di accesso**.

1. Attivare l'opzione nella sezione relativa all'utente amministratore per abilitare l'utente amministratore per questo Registro Container.
...
1. Nel pannello del repository, nella sezione **Impostazioni**, selezionare **Chiavi di accesso**.

1. Nel pannello **Chiave di accesso** selezionare la casella di controllo **Utente amministratore**.

1. Nel pannello del repository, nella sezione **Servizi**, selezionare **Repository**. 

1. Nell'elenco dei repository selezionare **ipcheck**.

1. Nel riquadro **ipcheck** selezionare il menu con i puntini di sospensione associato alla voce di tag **più recente** e quindi selezionare **Esegui istanza**.
...
1. Selezionare l'icona di **Cloud Shell** e quindi l'ambiente **Bash**.

1. Nel prompt dei comandi di **Cloud Shell** nel portale eseguire il comando seguente per installare l'estensione di App contenitore di Azure per l'interfaccia della riga di comando: 
...
1. Eseguire il comando seguente per registrare il provider **Microsoft.OperationalInsights** per l'area di lavoro Log Analytics di Monitoraggio di Azure, se non è stato usato in precedenza: 
    ...
    > **Nota**: La registrazione dello spazio dei nomi **Microsoft.App** e di **Microsoft.OperationalInsights** può richiedere alcuni minuti.
...
Dopo aver aggiornato l'interfaccia della riga di comando e aver creato le variabili di ambiente, è possibile creare un ambiente App contenitore e distribuire l'app contenitore.
...
1. Nel prompt dei comandi di **Cloud Shell** nel portale eseguire il comando seguente per creare un ambiente usando il comando di creazione `az containerapp env`: 
...
1. Nel prompt dei comandi di **Cloud Shell** nel portale eseguire il comando seguente per distribuire un'immagine di app contenitore di esempio usando il comando `az containerapp create`: 

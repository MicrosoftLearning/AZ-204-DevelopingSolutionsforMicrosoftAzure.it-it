...
#### Attività 3: Aprire il Cloud Shell e l'editor
...
#### Attività 6: Archiviare i metadati del Registro Contenitori
...
#### Attività 8: Convalidare l'immagine del contenitore in Registro Contenitori
...
### Esercizio 3: Creare un ambiente di app contenitore sicuro e distribuire l'app contenitore.
...
1. Nel prompt dei comandi **Cloud Shell** nel portale eseguire il comando seguente per installare l'estensione App Azure Container per l'interfaccia della riga di comando: 
...
1. Eseguire il comando seguente per registrare il provider **Microsoft.OperationalInsights** per l'area di lavoro Log Analytics di Monitoraggio di Azure se non è stata usata prima: 

    ```bash
    az provider register --namespace Microsoft.OperationalInsights
    ```

    > **Nota**: la registrazione dello spazio dei nomi **Microsoft.App** e **Microsoft.OperationalInsights** può richiedere alcuni minuti per completare.
...
Con le variabili di interfaccia della riga di comando aggiornate e di ambiente create, è possibile creare un ambiente app contenitore e distribuire l'app contenitore.

#### Attività 2: Creare un ambiente

Un ambiente in App Azure Container crea un limite sicuro intorno a un gruppo di app del contenitore. Le app contenitore distribuite nello stesso ambiente vengono distribuite nella stessa rete virtuale e scrivono i log nella stessa area di lavoro Log Analytics.

1. Nel prompt dei comandi **Cloud Shell** nel portale eseguire il comando seguente per creare un ambiente usando il `az containerapp env` comando create: 
...
1. Nel prompt dei comandi **Cloud Shell** nel portale eseguire il comando seguente per distribuire un'immagine del contenitore di app di esempio usando il `az containerapp create` comando : 
...
1. Selezionare il collegamento restituito dal `az containerapp create` comando per verificare che l'app contenitore sia in esecuzione.

...
In questo lab si apprenderà come usare Azure Storage SDK per accedere ai contenitori di Archiviazione di Azure all'interno di un'applicazione C#. Si apprenderà anche come accedere ai metadati ed esporre le informazioni URI per ottenere l'accesso al contenuto dei contenitori nell'account di archiviazione. 

Infine, per garantire la conformità alle normative aziendali, si implementerà l'accesso sicuro generando firme di accesso condiviso e criteri relativi al ciclo di vita dei dati. Si testerà anche l'accesso al contenuto tramite un sito Web statico.
...
1.  Nella finestra **Carica BLOB** eseguire le azioni seguenti e quindi selezionare **Carica**:
    ...
    | Impostazione | Azione |
    | -- | -- |
    | Sezione **File** | Selezionare **Sfoglia per i file** o usare la funzionalità trascinamento e rilascio |
